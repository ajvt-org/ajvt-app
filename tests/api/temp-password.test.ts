...
describe("temporary passwords", () => {
  ...
  it("signs in on a live temporary password and flags the token", async () => {
    await withTempPassword("123456", new Date(Date.now() + HOUR));

    const res = await LOGIN(post("/api/auth/login", { phone: "22334455", password: "123456" }));
  ...
  it("locks the rest of the app while the password is temporary", async () => {
    const user = await withTempPassword("123456", new Date(Date.now() + HOUR));
    await signInAs(user);
  ...
  it("changes the password without being given the old one", async () => {
    ...
    const res = await CHANGE(post("/api/user/password", { newPassword: "chosen" }));

    expect(res.status).toBe(200);
    const after = await prisma.user.findUniqueOrThrow({ where: { id: user.id } });
    expect(await bcrypt.compare("chosen", after.password)).toBe(true);
  });
  ...
  it("clears the expiry, so the lock lifts", async () => {
    ...
    await CHANGE(post("/api/user/password", { newPassword: "chosen" }));
  ...
  it("kills the temporary password once a real one is chosen", async () => {
    ...
    const old = await LOGIN(post("/api/auth/login", { phone: "22334455", password: "123456" }));
    const chosen = await LOGIN(post("/api/auth/login", { phone: "22334455", password: "chosen" }));
  ...
  it("refuses to keep the temporary password as the real one", async () => {
    ...
    expect((await CHANGE(post("/api/user/password", { newPassword: "123456" }))).status).toBe(400);
  });
