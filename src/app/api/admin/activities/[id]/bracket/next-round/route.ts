...
export async function POST(
  ...
  try {
    ...
    const bracketMatches = await prisma.match.findMany({
      ...
      select: {
        id: true, bracketRound: true, order: true, homeTeamId: true, awayTeamId: true,
        homeScore: true, awayScore: true, homePenalties: true, awayPenalties: true, status: true,
      },
    });

    if (bracketMatches.length === 0) {
      return NextResponse.json({ error: "لم تبدأ القرعة بعد — أجرِ القرعة أو ولّد نصف النهائي أولاً" }, { status: 400 });
    }

    const maxRound = bracketMatches[0].bracketRound as number;
    const currentRound = bracketMatches.filter((m) => m.bracketRound === maxRound).sort((a, b) => a.order - b.order);

    if (currentRound.length === 1) {
      return NextResponse.json({ error: "هذا الدور هو النهائي بالفعل" }, { status: 409 });
    }
    if (currentRound.length % 2 !== 0) {
      return NextResponse.json({ error: "عدد مباريات هذا الدور فردي — تحقّق من عدم حذف إحدى المباريات يدوياً" }, { status: 409 });
    }
    ...
    for (const m of currentRound) {
      ...
      if (!winner) {
        return NextResponse.json({ error: "إحدى مباريات هذا الدور تعادلت دون ركلات ترجيح — أدخل نتيجة ركلات الترجيح أولاً" }, { status: 409 });
      }
      winners.push(winner);
    }
    ...
    await logAction(session.username, "GENERATE_BRACKET_NEXT_ROUND", `${label} — ${data.length} مباراة`);
