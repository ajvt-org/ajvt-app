...
export async function POST(
  ...
  try {
    ...
    const groups = await prisma.group.findMany({ where: { activityId: id }, orderBy: { createdAt: "asc" }, select: { id: true, name: true } });
    if (groups.length !== 2) {
      return NextResponse.json({ error: "يجب أن يكون هناك مجموعتان بالضبط (المجموعة أ والمجموعة ب) لتوليد نصف النهائي التلقائي" }, { status: 400 });
    }

    const [teams, matches] = await Promise.all([
      prisma.team.findMany({ where: { activityId: id }, select: { id: true, name: true, groupId: true } }),
    ...
    if (leagueMatches.some((m) => m.status !== "PLAYED")) {
      return NextResponse.json({ error: "أكمل جميع نتائج دور المجموعات أولاً قبل توليد نصف النهائي" }, { status: 409 });
    }
    ...
    if (existingBracket) {
      return NextResponse.json({ error: "توجد مباريات إقصائية بالفعل لهذه البطولة — احذفها أولاً لإعادة التوليد" }, { status: 409 });
    }
    ...
    const data = [
      { activityId: id, homeTeamId: standingsA[0].teamId, awayTeamId: standingsB[1].teamId, isKnockout: true, bracketRound: 1, round: label, order: nextOrder++ },
      { activityId: id, homeTeamId: standingsB[0].teamId, awayTeamId: standingsA[1].teamId, isKnockout: true, bracketRound: 1, round: label, order: nextOrder++ },
    ];

    await prisma.match.createMany({ data });
    await logAction(session.username, "GENERATE_BRACKET_SEMIS", `${groups[0].name} × ${groups[1].name}`);
