...
export const activityUpdateSchema = z
  .object({
    ...
    partsPerMatch: z.number().int().nullish(),
    matchEnding: z.enum(["PLAY_ALL", "FIRST_TO"], INVALID).nullish(),
    partsToWin: z.number().int().nullish(),
    partDecision: z.enum(["OUTCOME", "POINTS", "SCORE"], INVALID).nullish(),
    partTarget: z.number().int().nullish(),
    partWord: z.string(INVALID).nullish(),
    partsWord: z.string(INVALID).nullish(),
    hasColours: z.boolean().optional(),
    firstColourWord: z.string(INVALID).nullish(),
    secondColourWord: z.string(INVALID).nullish(),
