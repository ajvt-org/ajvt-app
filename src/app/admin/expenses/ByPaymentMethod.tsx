...
import { byPaymentMethod as texts } from "@/lib/texts";
...
function Detail({ detail }: { detail: MethodDetail }) {
  return (
    ...
    >
      <EntryList title={texts.membership} entries={detail.intisab} />
      <EntryList title={texts.support} entries={detail.daem} />
      {detail.anonymousTotal > 0 && (
        <div
          className="flex items-center justify-between text-xs pt-1.5"
          style={{ borderTop: "1px solid var(--mint-100)" }}
        >
          <span style={{ color: "var(--text-muted)" }}>{texts.anonymous}</span>
