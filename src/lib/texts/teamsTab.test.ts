...
describe("teams tab texts", () => {
  ...
  it("counts the players of a team", () => {
    expect(teamsTab.rosterCount(0)).toBe("0 لاعبين");
    expect(teamsTab.rosterCount(1)).toBe("لاعب");
  ...
  it("counts the join requests waiting on a team", () => {
    expect(teamsTab.requestCount(0)).toBe("0 طلبات انضمام");
    expect(teamsTab.requestCount(1)).toBe("طلب انضمام");
  ...
  it("counts the invitations nobody answered", () => {
    expect(teamsTab.invitationCount(0)).toBe("0 دعوات لم يُرد عليها");
    expect(teamsTab.invitationCount(1)).toBe("دعوة لم يُرد عليها");
    expect(teamsTab.invitationCount(2)).toBe("دعوتان لم يُرد عليهما");
    expect(teamsTab.invitationCount(3)).toBe("3 دعوات لم يُرد عليها");
    expect(teamsTab.invitationCount(11)).toBe("11 دعوةً لم يُرد عليها");
  });
