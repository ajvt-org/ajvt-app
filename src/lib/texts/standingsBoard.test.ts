...
describe("standings board texts", () => {
  it("weaves the rank and the total into a member's own place", () => {
    expect(standingsBoard.myPlace(14, 20)).toBe("ترتيبك 14 بمجموع 20 نقطةً");
  });

  it("counts the points of a place", () => {
    expect(standingsBoard.myPlace(1, 1)).toContain("بمجموع نقطة");
    expect(standingsBoard.myPlace(2, 2)).toContain("نقطتان");
