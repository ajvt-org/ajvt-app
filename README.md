...
Merging the release tags it, publishes the notes, and puts it in production. The service has auto-deploy on for `master` and waits for the checks to pass, so the deploy starts on its own once the merge is green and nobody presses anything. It takes a few minutes and it can fail, so check the service rather than assuming a fix went live with the merge.

Every boot runs `prisma migrate deploy` and the seed, so a deploy migrates the production database. That is the reason releases are a deliberate merge rather than every merge, and the reason the build has to pass before one can happen. The merge is the decision, and everything after it — the tag, the notes, the deploy and the migrations — is a consequence.
